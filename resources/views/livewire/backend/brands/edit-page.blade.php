<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-black text-text-main tracking-tight">Sửa thương hiệu</h1>
            <p class="text-sm text-text-muted mt-1">Cập nhật thông tin thương hiệu '{{ $name }}'.</p>
        </div>
                    ...
                    <div class="relative group cursor-pointer">
                        <div class="w-full h-40 rounded-xl border-2 border-dashed border-border-glass bg-bg-surface flex flex-col items-center justify-center gap-2 group-hover:border-primary/50 transition-all overflow-hidden">
                            ...
                            @elseif ($currentLogoUrl)
                                <img src="{{ $currentLogoUrl }}" class="w-full h-full object-contain p-4">
                            @else
                                <i class="ti ti-photo-plus text-3xl text-text-muted/30"></i>
                                <span class="text-[10px] font-black text-text-muted uppercase tracking-widest">Tải logo mới lên</span>
                            @endif
                        </div>
