<section class="page space-y-8 pb-32">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            <div class="w-14 h-14 rounded-2xl bg-warning/10 flex items-center justify-center border border-warning/20 text-warning shadow-inner">
                 <x-mary-icon name="o-pencil-square" class="w-7 h-7" />
            </div>
            <div>
                <p class="text-[11px] font-black text-warning uppercase tracking-[0.2em] mb-1 opacity-80">Chỉnh sửa nội dung</p>
    ...
    <form wire:submit="save" class="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
        ...
        <div class="xl:col-span-8 space-y-8">
            ...
            <!-- Basic Information -->
            <x-mary-card title="Cập nhật thông tin" separator shadow class="glass-card rounded-3xl border-white/10 shadow-2xl">
                <div class="grid grid-cols-1 gap-8 p-2">
                    <x-mary-input 
                        ...
                        class="text-lg font-black tracking-tight focus:ring-warning/20" 
                    />
        ...
        <div class="xl:col-span-4 space-y-8">
            ...
            <x-mary-card title="Ảnh đại diện" separator shadow class="glass-card rounded-3xl border-white/10 shadow-2xl">
                <div class="p-2">
                    <x-mary-file wire:model="form.image" label="" hint="Đề xuất: 800x600px" crop-after-change>
                        <div class="relative group cursor-pointer overflow-hidden rounded-2xl border-2 border-dashed border-white/10 hover:border-warning/50 transition-all duration-300 aspect-square flex items-center justify-center bg-base-200/30">
                            @if($form->image)
                                <img src="{{ $form->image->temporaryUrl() }}" class="object-cover w-full h-full" />
                            @elseif($currentImageUrl)
                                <img src="{{ $currentImageUrl }}" class="object-cover w-full h-full" />
                            @else
                                <div class="flex flex-col items-center gap-2">
                                    <x-mary-icon name="o-cloud-arrow-up" class="w-10 h-10 text-text-muted/20" />
                                    <span class="text-[10px] font-black uppercase tracking-widest text-text-muted/40 group-hover:text-warning transition-colors">Thay đổi hình ảnh</span>
                                </div>
                            @endif
                        </div>
                    </x-mary-file>
        ...
        <div class="fixed bottom-8 left-1/2 -translate-x-1/2 w-full max-w-4xl px-4 z-50">
            <div class="backdrop-blur-2xl bg-base-100/60 border border-white/20 p-4 rounded-3xl shadow-2xl flex items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 rounded-2xl bg-warning/10 flex items-center justify-center text-warning font-bold shadow-inner border border-warning/20">
                        <x-mary-icon name="o-arrow-path" class="w-6 h-6" />
                    </div>
                    <div class="flex flex-col">
                        <span class="text-[10px] font-black uppercase text-warning tracking-[0.2em] opacity-80">ID: #{{ $form->category->id }}</span>
                        <span class="text-base font-black text-text-main truncate max-w-[200px] tracking-tight">{{ $form->name }}</span>
                    </div>
                </div>
                
                <div class="flex items-center gap-3">
                    <x-mary-button label="Hủy" link="{{ route('backend.categories.index', $form->type) }}" class="btn-ghost font-bold" />
                    <x-mary-button label="Cập nhật thay đổi ✨" type="submit" class="btn-warning shadow-xl shadow-warning/30 font-black px-10 h-12 rounded-2xl" spinner="save" />
                </div>
            </div>
        </div>
    </form>
