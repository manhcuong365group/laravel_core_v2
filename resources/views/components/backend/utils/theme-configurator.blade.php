...
}" x-init="init()" class="fixed right-6 bottom-24 z-50">

    <!-- Toggle Button -->
    <button @click="configOpen = true"
        class="w-14 h-14 flex items-center justify-center rounded-2xl bg-primary text-white shadow-2xl shadow-primary/40 hover:scale-110 transition-all active:scale-95 group">
        <i class="ti ti-settings text-2xl animate-spin-slow group-hover:pause"></i>
    </button>
    ...
    <template x-teleport="body">
        <div x-show="configOpen" x-cloak class="fixed inset-0 z-60">
            ...
            <div x-show="configOpen" x-transition:enter="transition ease-out duration-300 transform"
                ...
                <!-- Sidebar Style -->
                <div class="space-y-4 mb-8">
                    <label class="text-xs font-black text-text-muted uppercase tracking-widest">Kiểu Sidebar</label>
                    <div class="grid grid-cols-2 gap-3">
                        <button
                            @click="sidebarStyle = 'glass'; localStorage.setItem('sidebarStyle', 'glass'); document.documentElement.style.setProperty('--sidebar-bg', darkMode ? '#0b0b0f' : '#ffffff')"
                            class="flex flex-col items-center gap-2 p-3 rounded-2xl border-2 transition-all"
                            :class="sidebarStyle === 'glass' ? 'border-primary bg-primary/5' : 'border-border-glass'">
                            <span class="text-[10px] font-black uppercase tracking-tight"
                                :class="sidebarStyle === 'glass' ? 'text-primary' : 'text-text-muted'">Glass</span>
                        </button>
                        <button
                            @click="sidebarStyle = 'solid'; localStorage.setItem('sidebarStyle', 'solid'); document.documentElement.style.setProperty('--sidebar-bg', darkMode ? '#000000' : '#f8fafc')"
                            class="flex flex-col items-center gap-2 p-3 rounded-2xl border-2 transition-all"
                            :class="sidebarStyle === 'solid' ? 'border-primary bg-primary/5' : 'border-border-glass'">
                            <span class="text-[10px] font-black uppercase tracking-tight"
                                :class="sidebarStyle === 'solid' ? 'text-primary' : 'text-text-muted'">Solid</span>
                        </button>
                    </div>
                </div>
